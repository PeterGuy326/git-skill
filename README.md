...
|---|---|---|
| [`release-sop`](./skills/release-sop) | ✅ shipped | Drives any project through a 10-stage release SOP (project ID → identity → **SemVer version recommend** → PR DoR replay → Pre-flight → **Cut PR** rename `[Unreleased]` → `[X.Y.Z]` reviewed by a teammate → tag → CI watch → smoke → DOD). Auto-detects latest tag + reads `[Unreleased]` to recommend patch/minor/major then asks the Captain to confirm; CHANGELOG ships as a separate Cut PR (no self-approve; sole-maintainer exception requires PR-body declaration + 24h cool-off). Includes an **optional `release-please` automation track** that bots Step 1.5 + Cut PR body + tag push while preserving the human review gate. Auto-detects Go / Node / Python / Rust / Docker. |
